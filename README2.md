...
Blueprint: `app/api/admin/__init__.py` — **auth ปัจจุบันเป็น placeholder** (`@admin_required` ผ่านทุก request)
...
| Route | หน้าที่ |
|-------|--------|
| `/login`, `/register`, `/verify-otp` | auth UI (login ยังเป็น placeholder) |
| `/` | แดชบอร์ด |
| `/products` | สินค้า |
