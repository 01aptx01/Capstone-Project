...
export default function ReportCard({
  ...
  if (isFeatured) {
    return (
      <div className="relative overflow-hidden bg-[#10B981] rounded-[24px] p-6 text-white shadow-[0_20px_50px_rgba(16,185,129,0.2)]">
        <div className="flex justify-between items-start mb-4">
          <div className="text-[14px] font-bold opacity-90">{title}</div>
          {trend && (
            <div className="bg-white/20 backdrop-blur-md px-2 py-1 rounded-lg text-[12px] font-bold">
              {trendDirection === "up" ? "+" : ""}{trend}
            </div>
          )}
        </div>
        <div className="text-[42px] font-black leading-none mb-4">{value}</div>
        <div className="text-[12px] font-medium opacity-80">{subValue}</div>
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300">
      <div className="flex justify-between items-start mb-4">
        ...
        {trend && (
          <div className="flex items-center gap-1 font-bold text-[12px]" style={{ color: trendColor }}>
