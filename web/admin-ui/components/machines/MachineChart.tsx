...
const fallbackMonth = [2500, 3200, 2900, 3800];
...
export default function MachineChart() {
  ...
  useEffect(() => {
    ...
    const newData = Array.from({length: count}, () => Math.floor(Math.random() * 3000) + 500);
    setHistData(newData);
  ...
  const { projPath, projPoint } = useMemo(() => {
    ...
    const padding = 16;
    const innerWidth = Math.max(chartWidth - padding * 2, 10);
    const projX = padding + innerWidth; 
  ...
  const labels = (() => {
    if (viewMode === "historical") return histLabels;
            ...
            {chartWidth > 0 && (
              ...
              >
                <defs>
                  ...
                  <linearGradient id="strokeGrad2" x1="0" x2="1">
                    <stop offset="0%" stopColor="var(--primary)" />
                    <stop offset="100%" stopColor="var(--primary)" />
                  </linearGradient>
                </defs>

                {areaD && <motion.path initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} d={areaD} fill="url(#areaGrad2)" stroke="none" />}
                {pathD && (
                  <motion.path
